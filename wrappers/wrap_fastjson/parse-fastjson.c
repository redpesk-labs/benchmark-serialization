...
/// @brief Serialize a SensorVersion object using fastjson.
/// @param[in] sv The object to serialize.
/// @return The serialized result.
struct json_object* fastjson_serialize_SensorVersion(SensorVersion sv, enum Option_t opt)
{
...
/// @brief Deserialize a SensorVersion object using fastjson.
/// @param[in] input The object to deserialize.
/// @param[out] output The deserialized object.
/// @return @c EXIT_SUCCESS or @c EXIT_FAILURE
int fastjson_deserialize_SensorVersion(struct json_object* input, SensorVersion* output, enum Option_t opt)
{
...
/// @brief Serialize a SensorStatus object using fastjson.
/// @param[in] ss The object to serialize.
/// @return The serialized result.
struct json_object* fastjson_serialize_SensorStatus(SensorStatus ss, enum Option_t opt)
{
	struct json_object* out;
...
/// @brief Deserialize a SensorStatus object using fastjson.
/// @param[in] input The object to deserialize.
/// @param[out] output The deserialized object.
/// @return @c EXIT_SUCCESS or @c EXIT_FAILURE
int fastjson_deserialize_SensorStatus(struct json_object* input, SensorStatus* output, enum Option_t opt)
{
...
/// @brief Serialize a TargetStatus object using fastjson.
/// @param[in] ts The object to serialize.
/// @return The serialized result.
struct json_object* fastjson_serialize_TargetStatus(TargetStatus ts, enum Option_t opt)
{
	struct json_object* out;
...
/// @brief Deserialize a TargetStatus object using fastjson.
/// @param[in] input The object to deserialize.
/// @param[out] output The deserialized object.
/// @return @c EXIT_SUCCESS or @c EXIT_FAILURE
int fastjson_deserialize_TargetStatus(struct json_object* input, TargetStatus* output, enum Option_t opt)
{
...
/// @brief Serialize a TargetInfo object using fastjson.
/// @param[in] ts The object to serialize.
/// @return The serialized result.
struct json_object* fastjson_serialize_TargetInfo(TargetInfo ti, enum Option_t opt)
{
	struct json_object* out;
...
/// @brief Deserialize a TargetInfo object using fastjson.
/// @param[in] input The object to deserialize.
/// @param[out] output The deserialized object.
/// @return @c EXIT_SUCCESS or @c EXIT_FAILURE
int fastjson_deserialize_TargetInfo(struct json_object* input, TargetInfo* output, enum Option_t opt)
{
...
/// @brief Serialize a SensorData object using fastjson.
/// @param[in] ts The object to serialize.
/// @return The serialized result.
struct json_object* fastjson_serialize_SensorData(SensorData input, enum Option_t opt)
{
	json_object* out;

	if (opt == ARRAY) {
		out = json_object_new_array();
		json_object_array_add(out, fastjson_serialize_SensorVersion(input.version, opt));
		json_object_array_add(out, fastjson_serialize_SensorStatus(input.sStatus, opt));
		json_object_array_add(out, fastjson_serialize_TargetStatus(input.tStatus, opt));
		json_object_array_add(out, fastjson_serialize_TargetInfo(input.tInfo, opt));
		json_object_array_add(out, json_object_new_int(input.tInfoSize));		
	} else {
		out = json_object_new_object();
		json_object_object_add(out, "version", fastjson_serialize_SensorVersion(input.version, opt));
		json_object_object_add(out, "sStatus", fastjson_serialize_SensorStatus(input.sStatus, opt));
		json_object_object_add(out, "tStatus", fastjson_serialize_TargetStatus(input.tStatus, opt));
		json_object_object_add(out, "tInfo", fastjson_serialize_TargetInfo(input.tInfo, opt));
...
/// @brief Deserialize a SensorData object using fastjson.
/// @param[in] input The object to deserialize.
/// @param[out] output The deserialized object.
/// @return @c EXIT_SUCCESS or @c EXIT_FAILURE
int fastjson_deserialize_SensorData(struct json_object* input, SensorData* output, enum Option_t opt)
{

	if (opt == ARRAY) {
		if (fastjson_deserialize_SensorVersion(json_object_array_get_idx(input, 0), &output->version, opt)) return EXIT_FAILURE;
		if (fastjson_deserialize_SensorStatus(json_object_array_get_idx(input, 1), &output->sStatus, opt)) return EXIT_FAILURE;
		if (fastjson_deserialize_TargetStatus(json_object_array_get_idx(input, 2), &output->tStatus, opt)) return EXIT_FAILURE;
		if (fastjson_deserialize_TargetInfo(json_object_array_get_idx(input, 3), &output->tInfo, opt)) return EXIT_FAILURE;
		...
		if (fastjson_deserialize_SensorVersion(tmp, &output->version, opt)) return EXIT_FAILURE;

		if (!json_object_object_get_ex(input, "sStatus", &tmp)) return EXIT_FAILURE;
		if (fastjson_deserialize_SensorStatus(tmp, &output->sStatus, opt)) return EXIT_FAILURE;

		if (!json_object_object_get_ex(input, "tStatus", &tmp)) return EXIT_FAILURE;
		if (fastjson_deserialize_TargetStatus(tmp, &output->tStatus, opt)) return EXIT_FAILURE;

		if (!json_object_object_get_ex(input, "tInfo", &tmp)) return EXIT_FAILURE;
		if (fastjson_deserialize_TargetInfo(tmp, &output->tInfo, opt)) return EXIT_FAILURE;
...
/// @return @c EXIT_SUCCESS or @c EXIT_FAILURE.
int fastjson_print(void* ctx, void* data)
{
...
/// @return @c EXIT_SUCCESS or @c EXIT_FAILURE.
int fastjson_freeobject(void* ctx, void* data)
{
...
/// @return @c EXIT_SUCCESS or @c EXIT_FAILURE.
int fastjson_serialize(void* ctx, SensorData input, void** output)
{
	if (!output) return EXIT_FAILURE;
	enum Option_t opt = *((int *)ctx);
	*output = fastjson_serialize_SensorData(input, opt);
	return EXIT_SUCCESS;
}
...
/// @return @c EXIT_SUCCESS or @c EXIT_FAILURE.
int fastjson_deserialize(void* ctx, void* input, SensorData* output)
{
	if (!input || !output) return EXIT_FAILURE;
	enum Option_t opt = *((int *)ctx);
	return fastjson_deserialize_SensorData((struct json_object*)input, output, opt);
}

/// @brief Get the fastjson serializer.
/// @param[out] s The serializer.
void fastjson_get_serializer(Serializer* s)
{
	if (s)
	{
		s->context		= NULL;
		s->init			= fastjson_init;
		s->cleanup		= fastjson_cleanup;
		s->print		= fastjson_print;
		s->freeobject	= fastjson_freeobject;
		s->serialize	= fastjson_serialize;
		s->deserialize	= fastjson_deserialize;
	}
}